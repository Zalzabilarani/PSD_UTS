...
tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(
    ["Prediksi", "Dataset", "Ekstraksi Fitur", "Preprocessing", "Akurasi", "Reduksi PCA", "Grid Search"])
...
if uploaded_file is not None:
    ...
    if st.button("Cek Prediksi"):
        ...
        statistik = calculate_statistics(audio_path) #mengektraksi data yang di dev berparameter audio
...
with tab2:
    ...
    st.write("""<h3> Toronto emotional speech set (TESS) <h3>""",
             unsafe_allow_html=True)
    st.write("""<h5> A dataset for training emotion (7 cardinal emotions) classification in audio <>""",
             unsafe_allow_html=True)
...
with tab4:
    ...
    if scaler == 'Tanpa Normalisasi Data':
        ...
        normalized_data_zscore = pd.DataFrame(
            X_train_zscore, columns=X_train.columns)
        ...
        normalized_data_minmax = pd.DataFrame(
...
with tab5:
    ...
    plt.plot(list(accuracy_dict_zscore.keys()), list(
        accuracy_dict_zscore.values()), label='Z-score')
    plt.plot(list(accuracy_dict_minmax.keys()), list(
        accuracy_dict_minmax.values()), label='Min-Max')
    plt.xlabel('k Value')
    plt.ylabel('Accuracy')
    ...
    st.write(
        f"Best k value with Z-score : {best_k_zscore}, Accuracy : {accuracy_dict_zscore[best_k_zscore]*100:.2f}%")
    ...
    st.write(
        f"Best k value with Min-Max : {best_k_minmax}, Accuracy : {accuracy_dict_minmax[best_k_minmax]*100:.2f}%\n\n\n")
    ...
    if scaler == 'MinMax':
        st.write(
            " Accuracy Score Model KNN Setelah Proses Normalisasi dengan MinMax : ")
        # Membaca data dari file CSV
        df = pd.read_csv('hasil_statistik2.csv')
        ...
        k_values = st.slider("Choose the value of k:",
        ...
        st.write(
            f"Nilai k terbaik adalah {best_k} dengan akurasi {accuracy_dict[best_k]*100:.2f}%")

    elif scaler == "Zscore":
        st.write(
            " Accuracy Score Model KNN Setelah Proses Normalisasi dengan Zscore : ")
        # Membaca data dari file CSV
        df = pd.read_csv('hasil_statistik2.csv')
...
with tab6:
    ...
    if scaler == 'ReduksiMinMax':
        ...
        n_components = st.slider(
            "Choose the number of PCA components :", min_value=1, max_value=20, value=16, step=1)
        k_value = st.slider("Choose the value of K :",
                            min_value=1, max_value=100, value=9, step=1)
        ...
        st.write(
            f"Akurasi Model dengan n_components = {n_components} dan k = {k_value}: {accuracy:.4f}")
        ...
        data = {'Component': list(
            range(1, 21)), 'Best K': best_k_values, 'Accuracy': best_accuracies}
        df_result = pd.DataFrame(data)
        st.write(df_result)
        st.write(
            f"Nilai akurasi reduksi pca pada knn dengan minmax scaler yakni {accuracy*100:.2f}% dengan k {k} pada komponen {n}")
        ...
        n_components = st.slider(
            "Choose the number of PCA components :", min_value=1, max_value=20, value=14, step=1)
        k_value = st.slider("Choose the value of K :",
        ...
        st.write(df_result)
        st.write(
            f"Nilai akurasi reduksi pca pada knn dengan zscore scaler yakni {accuracy*100:.2f}% dengan k {k} pada komponen {n}")
...
with tab7:
    ...
    if scaler == 'Grid Search Z-Score':
        ...
        # Menggunakan data latih yang belum diproses PCA
        ...
        for n_components in range(X_train.shape[1], 0, -1):
            ...
            st.write(
                f"Accuracy dengan {n_components} PCA components: {grid_knn_pca * 100:.2f}%")
